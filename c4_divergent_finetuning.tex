...
It is divded into two sub-sections the first (\S \ref{c4:sec:og-loss}) shows the results of the standard inverted adversarial loss (Eq. \ref{eq:inverted-adv-loss}), the second (\S \ref{c4:sec:log-loss}) shows the results for the natural logarithm of the inverted adversarial loss (Eq. \ref{eq:ln-inverted-adv-loss}).
...
This results presented here demonstrate an efficient method of divergent fine-tuning, away from likelihood of the original training data. 
...
For the second loss function \ref{eq:ln-inverted-adv-loss}, which uses the natural log of the inverse adversial loss, there is clearly much less of a fixed point to which the optimiser is moving towards. 
...
By taking the log of the loss, this appears to keeps the range of the loss function within a much more constrained range.
...
As these loss scores are also tending towards negative numbers, thus when we take the log of this loss, these negative numbers become undefined (as taking the log of a negative number is always undefined).
...
Given the fact that images from these models have been used to make fake social media accounts \citep{satter2019spy} by spies trying to penetrate the American defense establishment, it is clear that StyleGAN generated images, at least in some instances, have crossed the threshold of the uncanny valley towards producing completely plausible and convincing images of people.
