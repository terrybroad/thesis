...
Section \ref{c7:sec:unstable_eq} details the reception of the series of artworks \textit{(un)stable equilibrium} that resulted from these experiments. 

In that initial paper, I documented 6 experiments, the outputs of each training run becoming the artworks \textit{(un)stable equilibrium 1:1} through to \textit{(un)stable equilibrium 1:6}  (Fig. \ref{fig:c3:original-experiments}). 
...
Section \ref{c7:sec:unstable_eq} details the reception and artistic presentation of the original training runs in more detail. 
...
That a generative model is simply a statistical model of a given data distribution, and nothing more. 
...
My first experiment took inspiration from the GAN framework  (Fig. \ref{subfig:c3:og-gan-diagram}) where a generator network imitates a training dataset, and the discriminator tries to tell them apart (Eq. \ref{eq:gan-loss}). 
...
My initial adaptation of this was to replace the training data with another generator  (Fig. \ref{subfig:c3:double-gen-diagram}). 
...
\begin{equation} 
    Adv = \min_{G_{1}}\max_{G_2}\max_{D}\mathbb{E}_{z\sim p_{\text{z}}(z)}[\log{D(G_{1}(z))}] +  \mathbb{E}_{z\sim p_{\text{z}}(z)}[1 - \log{D(G_{2}(z))}]
...
In this configuration, the visual results  (Fig. \ref{fig:c3:samples-no-col-var}) were not what I was hoping for.  
There is little diversity across the training batch for both generators. 
Essentially, this training regime suffered from mode collapse, which is a common failure state for GANs \citep{thanh2020catastrophic}.
...
Equation \ref{eq:gan-g1-col-var} shows the training objective for $G_
{1}$ and Equation \ref{eq:gan-g2-col-var} shows the training objective for $G_{2}$.
...
The visual results  (Fig. \ref{fig:c3:samples-col-var}) with the additional loss term are  much improved with respect to the variety across the generated data distribution  when compared to the previous experiment without the colour variance loss term  (Fig. \ref{fig:c3:samples-no-col-var}).
...
The total loss for both generators is given in Eqs. \ref{eq:g1-cosine-loss} \& \ref{eq:g2-cosine-loss}.
...
The Euclidean distance between two vectors is shown in Equation \ref{eq:euclid-dist}.
...
This is calculated across the mini-batch $\vec d_{bg_{1}}, \vec d_{bg_{2}}$ and the mean is taken to calculate the loss for the mini-batch. 
The total loss for both generators is given in Eqs. \ref{eq:g1-euclid-loss} \& \ref{eq:g2-euclid-loss}.
...
For this training setup $G_{1}$ is trained with Equation \ref{eq:g1-cosine-loss} and $G_{2}$ is trained with Equation \ref{eq:gan-g2-col-var}.
...
For this training setup $G_{1}$ is trained with Equation \ref{eq:g1-euclid-loss} and $G_{2}$ is trained with Equation \ref{eq:gan-g2-col-var}.
...
For this training setup $G_{1}$ is trained with Equation \ref{eq:g1-cosine-loss} and $G_{2}$ is trained with Equation \ref{eq:g2-euclid-loss}.
...
This however, was a happy accident, or at least, my aesthetic preferences were the `meta-heuristic' guiding me in my development of the code over the course of several weeks which finally led to creating a training ensemble of networks that created works that share this resemblance.
...
In addition, the results from the original training experiments  (Fig. \ref{fig:c3:original-experiments}), which became the series of artworks \textit{(un)stable equilibrium}, have had a significant artistic reception, which has been more significant than the technical contribution.
This artistic reception is detailed in Section \ref{c7:sec:unstable_eq}.

The following chapter builds on this work, though instead of focusing on training models from scratch, 
My next experiments explored fine-tuning models that had already been trained in a data divergent fashion.
