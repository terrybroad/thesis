...
In the end, I renderd a 4/5d tensor of perlin noise, \textbf{( CHECK details on this)}. This gave me the sequence of transformations that would smoothly change from one frame to the next. 
...
Unfortunately illness and the pandemic impeded my ability to do that. 
...
In the AutoLume-live system \citep{kraasch2022autolume,kraasch2023autolume} network bending is one several features integrated into a real-time GAN-based VJing system. 
...
Autolume can be operated using physical mixing desk style interface.
...
They optimise the weights of this new layer using the CLIP (Contrastive Language-Image Pretraining) \citep{radford2021learning} towards matching an pre-set text prompt (Figure \ref{fig:c7:differential-net-bend}).
